#!/usr/bin/env python3
"""
MRI Resampling Pipeline (XY ONLY - Z PRESERVED)

✅ Resamples only X and Y (in-plane)
✅ Keeps Z spacing and slice count unchanged
✅ Correct RAS ↔ LPS conversion
✅ Correct axis handling (SITK ↔ NumPy ↔ Nibabel)
✅ BSpline for images / Nearest for masks
"""
...
def sitk_to_nib(sitk_img):
    data_zyx = sitk.GetArrayFromImage(sitk_img)  # (Z,Y,X)
    data_xyz = np.transpose(data_zyx, (2, 1, 0))  # → (X,Y,Z)
...
# 🔧 XY-ONLY RESAMPLING
# -------------------------
def resample_xy_only(sitk_img, target_xy=(0.5, 0.5), is_mask=False):
    original_spacing = np.array(sitk_img.GetSpacing())  # (sx, sy, sz)
    original_size = np.array(sitk_img.GetSize())        # (nx, ny, nz)

    new_spacing = np.array([
        target_xy[0],
        target_xy[1],
        original_spacing[2]  # keep Z spacing
    ])

    new_size = np.array([
        int(round(original_size[0] * (original_spacing[0] / new_spacing[0]))),
        int(round(original_size[1] * (original_spacing[1] / new_spacing[1]))),
        int(original_size[2])  # keep Z slices
    ])
...
# 🧠 PROCESS ONE FILE
# -------------------------
def process_single(in_path, out_path, target_xy=(0.5, 0.5), force_ras=True):
    try:
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("🚀 MRI RESAMPLING (XY ONLY - Z PRESERVED)")
    logging.info("=" * 80)
    ...
    with ProcessPoolExecutor(max_workers=2) as executor:
        futures = {
            executor.submit(process_single, in_p, out_p, target_xy): in_p
